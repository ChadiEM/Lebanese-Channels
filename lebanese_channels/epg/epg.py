...
def get_epg_channel_declaration(channel: Channel):
    return '<channel id="' + str(
        channel.id) + '"><display-name lang="en">' + channel.get_name() + '</display-name></channel>'
...
def get_epg_as_xml(channel: Channel):
    ...
    if epg_data is None:
        return ''
    return __build_response(epg_data, channel.id)


def __build_response(program_data_list: List[ProgramData], channel_id: int):
    response = ''
